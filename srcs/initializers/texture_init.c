...
/*   Updated: 2025/12/19 11:31:20 by aijadid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/cub3d.h"

void	image_converter_2(t_mlx *mlx)
{
	t_image	*textures;

	textures = mlx->map.textures;
	textures[EA].img_ptr = mlx_xpm_file_to_image(mlx->mlx,
			mlx->map.textures[EA].path, &(textures[EA].width),
			&(textures[EA].height));
	if (!textures[EA].img_ptr)
		return (error(&(mlx->map), "Closing Window !Bad Texture"));
...
void	image_address(t_mlx *mlx)
{
	...
	textures = mlx->map.textures;
	textures[NO].addr = mlx_get_data_addr(textures[NO].img_ptr,
			&(textures[NO].bpp), &(textures[NO].line_len),
			&(textures[NO].endian));
	if (!textures[NO].addr)
		return (error(&(mlx->map), "Closing Window !Address Get Failed"));
